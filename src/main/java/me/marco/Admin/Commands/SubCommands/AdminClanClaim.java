...
public class AdminClanClaim extends AdminCommand implements ICommand {
    ...
    public AdminClanClaim(Core instance){
    ...
    public void run(Player player, String[] args) {
        if(args.length == 1){
            chat.sendClans(player, "You must enter a clan to claim for" + getCommandExample());
            return;
        }
        ...
        Clan clan = clanManager.clanExists(clanName) ? clanManager.getClan(clanName) : clanManager.getAdminClan(clanName);
    ...
    public String getDescription() {
        return chat.commandDescription("Claim land for any clan");
    }

    public String getCommandExample() {
        return chat.commandExample("x claim <clanname>");
    }
